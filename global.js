window.onload = function (){
  ...
  fHandle = graphicDoc.getElementById("faucet_handle");
  ...
  //    Animations
  //--------------------------------------------
  TweenMax.to(fHandle, 4, { repeat: -1, rotationY: 360, transformOrigin: "50% 50%", onComplete: console.log("Handle moved") });
  TweenMax.to(droplet, 1.5, { opacity:1, delay:.5, y: 320, x:20, ease: Power1.easeInOut, repeat: -1, fill:"blue", onComplete: console.log("complete") });
  ...
  var thing = graphicDoc.querySelectorAll("#jacks").addEventListener("click", keepScore);
...
function startParachute(){
  TweenMax.to(parachute, 8, {  y: -208, x:245, ease: Power1.easeInOut, onComplete: startGear });
}  
...
function bagLift(){
  ...
  TweenMax.to(bPin, .8, {rotation:75, transformOrigin:"100% 100%"});
